...
std::stringstream ss;
enum TrackingState { SEARCHING, TRACKING };
...
int main(int argc, char **argv)
{
  ...
  nh.param<float>("target_x_mission_3", target_x_mission_3,15);
  nh.param<float>("target_y_mission_3", target_y_mission_3,0);
  ...
  nh.param<float>("kk_xy", kk_xy, 10.0);
  ...
  nh.param<double>("k_xy", k_xy, 0.0005);
  nh.param<float>("error_max_image", error_max_image, 10);
    ...
    switch (mission_num)
    {
      ...
      case 3:
        ...
        { 
          ...
          //1. 更新雷达点云数据，存储在Laser中,并计算四向最小距离
          ros::spinOnce();
          collision_avoidance(target_x_mission_3,target_y_mission_3);
        ...
        break;
        //move to objection detect
        case 4:
          if(control_by_vel(0.0, -1.5, 16.0, -3.0, ALTITUDE, target_yaw, err_max)){
            ROS_INFO("到达目标点，巡航点任务完成");
            mission_num = 5;
          }
          break;
        case 5:
          ...
          break;
        case 6:
          if(control_by_vel(0.0, 0.4, 24.0, -1.2, ALTITUDE, target_yaw, 2*err_max)){
            ROS_INFO("到达目标点，巡航点任务完成");
            mission_num = 7;
          }
          break;
        ...
        case 8:
            if(mission_pos_cruise(35.0, 1.0, ALTITUDE, target_yaw, err_max)){
              ROS_INFO("REACH LAND POINT");
              mission_num = 100;
              break;
            }
  ...
  //land
  while(ros::ok()){//降落
    ROS_INFO("AUTO.LAND");if(mission_pos_cruise(35.0, 1.0, init_position_z_take_off+0.5, target_yaw, err_max)){
      offb_set_mode.request.custom_mode = "AUTO.LAND";
      set_mode_client.call(offb_set_mode);
      ros::Duration(10.0).sleep();
      ROS_WARN("LAND COMPLETE");
      exit(0);
    }
